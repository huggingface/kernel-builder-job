...
| `image`     | no       | `ghcr.io/huggingface/kernel-builder-job:latest` | Docker image with nix + `kernel-builder` |
...
pull requests get a build-only validation, and pushes to `main` publish
`ghcr.io/huggingface/kernel-builder-job:latest`, which the action uses by
default. Each push is also tagged `sha-<commit>` — an immutable reference for
pinning a known-good image via the `image` input or rolling back a bad push
(retag it as `latest`).
